...
# source parameter of your own(change it)
distance = 2.92*u.kpc # distance towards to your source
d = distance.cgs.value

# data should be derived from ds9
# depend on your source
flatfield = np.array([])*10**6*(14/3600*np.pi/180)**2*u.Jy
fluxData = np.array([])*10**6*(14/3600*np.pi/180)**2*u.Jy
...
y = greybody(xFreq,Tdust=sedResult.params['Tdust'].value,mass=sedResult.params['mass'].value)*u.g/u.s/u.s

ax  = fig.add_axes([0.1,0.1,0.8,0.6])
ax.plot(x,y.to(u.Jy).value, '-k')
ax.plot(x,y2018.to(u.Jy).value, '-b')
...
ax.text(10**2.7, 10**(np.log10(ymax)-(np.log(ymax)-np.log(ymin))*0.04), 
        'ID: your source name', horizontalalignment='left')
